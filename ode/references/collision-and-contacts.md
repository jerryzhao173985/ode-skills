...
- **At creation:** `dGeomSetData(geom, owner)` stores a `void*` on the geom (e.g. your owner object — the
  body wrapper or a material struct you set with `dGeomSetData`).
- **In the near-callback:** `dGeomGetData(o1)` / `dGeomGetData(o2)` recover both owners, **combine** their
  ...
  combine is **your model's choice** — `mu = min(mu1, mu2)` is the common engine default, but a material model
  may instead combine `mu` as a **product of per-surface roughness** (`mu = roughness1 · roughness2`), so that
  two rough<1 surfaces correctly give *low* friction, and derive `soft_erp`/`soft_cfm` from series springs. The
