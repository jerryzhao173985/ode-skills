...
- **Callback args are SAMPLE INDICES, not coordinates** — in `dReal hf_cb(void*, int x, int z)` the `x`/`z` are integer grid indices (`x` in `[0, widthSamples-1]`, `z` in `[0, depthSamples-1]`), NOT a world/local distance (`include/ode/collision.h:1114-1117`: "the index of a sample … in the range zero to (nWidthSamples-1)"). To author procedural terrain at a chosen scale, normalize the index by the sample count first — `ode/demo/demo_heightfield.cpp:158` uses `fx = (x - (widthSamples-1)/2) / (widthSamples-1)` (centred, `[-0.5, 0.5]`); multiply by `width` for metres. The return is then transformed by the `Build*` `scale`/`offset`. Treating the args as world coordinates silently yields terrain at the wrong scale.
