...
- **Callback args are SAMPLE INDICES, not coordinates** — in `dReal hf_cb(void*, int x, int z)` the `x`/`z` are integer grid indices (`x` in `[0, widthSamples-1]`, `z` in `[0, depthSamples-1]`), NOT a world/local distance (`include/ode/collision.h:1114-1117`: "the index of a sample … in the range zero to (nWidthSamples-1)"). To author procedural terrain at a chosen scale, the per-sample metric spacing is `width/(widthSamples-1)` (`ode/src/heightfield.cpp:159`). To center a procedural field use `fx = (dReal)x/(widthSamples-1) - 0.5` — maps `x` to `[-0.5, 0.5]` for ANY sample count — then multiply by `width` for metres. (`ode/demo/demo_heightfield.cpp:158` uses the equivalent `(x-(widthSamples-1)/2)/(widthSamples-1)` for its odd 15×31 grid; that integer `/2` off-centers an EVEN count, so prefer the `x/(N-1)-0.5` form.) The return is then transformed by the `Build*` `scale`/`offset`. Treating the args as world coordinates silently yields terrain at the wrong scale.
