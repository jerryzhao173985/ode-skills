...
- **Constraints have lifetimes — including ones you change at runtime.** Joints are persistent, motors impose
  a target each step, contact joints last one step — and you can **create or destroy a joint mid-simulation**
  to grasp and release an object (`references/cpp-patterns.md` → runtime grasp/release). "ODE is a constraint
  solver" is the whole model; a constraint's lifetime is a first-class tool, not a fixed property.
