...
- **Documented but never field-exercised (a card shipped, never built+ran across the 16 field sessions).** These have full reference cards but no headless field run — trust them at card level and field-test before relying: restitution/bounce contacts (`dContactBounce`), joint force feedback (`dJointSetFeedback`), compound bodies (`dGeomSetOffsetPosition`/`dGeomSetOffsetRotation` + `dMassAdd`/`dMassRotate`/`dMassTranslate`), the Plane2D joint, DIF export (`dWorldExportDIF`), convex geoms (`dCreateConvex`, needs `ODE_WITH_LIBCCD=ON`), the heightfield/trimesh array builders (`*BuildByte`/`Short`/`Single`/`Double`), seven joints with cards but no field run (Piston, PR, PU, DBall, DHinge, LMotor, Null), nested sub-space collision (`dSpaceCollide2` descent + `dGeomSetData` owner-recovery), and the optional C++ wrapper classes (`odecpp.h` — the raw C API was used throughout instead). The exercised surface (world/body/geom lifecycle, the collision loop, ball/hinge/hinge2/slider/universal/fixed/amotor/transmission/contact joints, the 3-level threading API) IS field-verified — see `references/real-world-usage.md` for who exercises what in production.
