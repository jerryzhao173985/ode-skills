...
| 20 | Reading `contact.surface.motion1/2/N` (or its sign) and assuming it tracks broadphase `o1`/`o2` order | The sign is set by contact **body-polarity**: when the belt is the **body-less** geom (the contact is attached `dJOINT_REVERSE`), negate `motionN` and `motion2` but **NOT** `motion1` (`fdir1` is yours); if both geoms have bodies, probe the sign. Use the `dPlaneSpace` recipe in `foundations/contacts-and-friction.md` | The normal points into body1 and is negated only when the contact is reversed (`contact.h:81-84`; `ode/src/joints/contact.cpp:162-163`), so `motion2`/`motionN` (ODE-frame) flip while `motion1` (your `fdir1`) does not — it is NOT a raw `o1`/`o2` swap. Full rule + recipe in `foundations/contacts-and-friction.md`. |
...
| 23 | Calling `dBodySetPosition(b,…)` every step to move/drag a dynamic body | For scripted motion that must push others, make the body **kinematic** (`dBodySetKinematic`, `objects.h:1410`) and drive `dBodySetLinearVel(b,(target−cur)/dt)` **before** the step; reserve `dBodySetPosition` for one-shot setup/reset (then also zero the velocities) | `dBodySetPosition` (`objects.h:1045`) **teleports** the CoM: it skips the integrator's swept collision (the body tunnels / generates no contacts along the path) and sets **no** velocity (`dBodyGetLinearVel` `objects.h:1141` is unchanged), so any contact it does hit transfers wrong/zero momentum. A kinematic body is velocity-driven — it affects others but is not moved by forces (`references/foundations/mental-model.md`, `references/bodies-and-mass.md`). |
